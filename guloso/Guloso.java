...
public class Guloso {
    ...
    public void gulosoMontagem() {
        ...
        grafo.insereAresta(this.L1.getTempoEst().length - 2, grafo.numVertices() - 1,
                this.L1.getTempoEst()[this.L1.getTempoEst().length - 1]);
        ...
        for (int i = 0; i < this.L2.getTempoEst().length - 2; i++) {
            grafo.insereAresta(i + this.L1.getTempoEst().length - 1, i + this.L1.getTempoEst().length,
                    this.L2.getTempoEst()[i + 2]);
        }

        // transporta entre linhas
        for (int i = 0; i < this.L1.getTempoTrans().length; i++) {
            grafo.insereAresta(i + 1, i + this.L1.getTempoEst().length,
                    this.L1.getTempoTrans()[i] + this.L2.getTempoEst()[i + 2]);
            grafo.insereAresta(i + this.L1.getTempoEst().length - 1, i + 2,
                    this.L2.getTempoTrans()[i] + this.L1.getTempoEst()[i + 2]);
        }
    }
